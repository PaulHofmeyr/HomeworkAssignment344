#!/usr/bin/env python3
"""
Regenerate hole-05 turf wall vertices in TurfWall.cpp from hole-5-turf-wall.csv.

Each CSV object is one separate wall segment (two walls for hole 5).

Usage:
  python3 convert_hole05_walls.py [path-to.csv]
...
DEFAULT_CSV = ROOT / "hole-5-turf-wall.csv"
...
BEGIN = "// BEGIN HOLE05_TURF_WALL_DATA (auto-generated — do not edit by hand)"
END = "// END HOLE05_TURF_WALL_DATA"
...
def emit_cpp(segments: OrderedDict[str, list[tuple[int, int]]], source: str) -> str:
    ...
    lines.append("static void addHole05TurfWallSegments(TurfWall* wall)")
    lines.append("{")
...
def ensure_markers(text: str) -> str:
    if BEGIN in text:
        return text
    needle = "// BEGIN HOLE10_TURF_WALL_DATA"
    if needle not in text:
        raise SystemExit("Could not find HOLE10 marker to insert HOLE05 block")
