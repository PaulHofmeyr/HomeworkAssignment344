#!/usr/bin/env python3
"""
Regenerate hole-18 turf wall vertices in TurfWall.cpp from hole-18-turf-wall.csv.

Each CSV object is one separate wall segment (three walls for hole 18).

Usage:
  python3 convert_hole18_walls.py [path-to.csv]
...
DEFAULT_CSV = ROOT / "hole-18-turf-wall.csv"
...
BEGIN = "// BEGIN HOLE18_TURF_WALL_DATA (auto-generated — do not edit by hand)"
END = "// END HOLE18_TURF_WALL_DATA"
...
def emit_cpp(segments: OrderedDict[str, list[tuple[int, int]]], source: str) -> str:
    ...
    lines.append("static void addHole18TurfWallSegments(TurfWall* wall)")
    lines.append("{")
...
def ensure_markers(text: str) -> str:
    if BEGIN in text:
        return text
    needle = "// BEGIN HOLE05_TURF_WALL_DATA"
    if needle not in text:
        raise SystemExit("Could not find HOLE05 marker to insert HOLE18 block")
