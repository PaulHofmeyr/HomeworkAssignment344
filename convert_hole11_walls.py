#!/usr/bin/env python3
"""
Regenerate hole-11 turf wall vertices in TurfWall.cpp from hole-11-turf-wall.csv.

Each CSV object is one separate wall segment (two walls for hole 11).

Usage:
  python3 convert_hole11_walls.py [path-to.csv]
...
DEFAULT_CSV = ROOT / "hole-11-turf-wall.csv"
...
BEGIN = "// BEGIN HOLE11_TURF_WALL_DATA (auto-generated — do not edit by hand)"
END = "// END HOLE11_TURF_WALL_DATA"
...
def emit_cpp(segments: OrderedDict[str, list[tuple[int, int]]], source: str) -> str:
    ...
    lines.append("static void addHole11TurfWallSegments(TurfWall* wall)")
    lines.append("{")
...
def ensure_markers(text: str) -> str:
    if BEGIN in text:
        return text
    needle = "// BEGIN HOLE01_HOLE09_TURF_WALL_DATA"
    if needle not in text:
        raise SystemExit("Could not find HOLE01/09 marker to insert HOLE11 block")
