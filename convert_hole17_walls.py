#!/usr/bin/env python3
"""Regenerate hole-17 turf walls in TurfWall.cpp from hole-17-turf-wall.csv."""
...
DEFAULT_CSV = ROOT / "hole-17-turf-wall.csv"
BEGIN = "// BEGIN HOLE17_TURF_WALL_DATA (auto-generated — do not edit by hand)"
END = "// END HOLE17_TURF_WALL_DATA"
...
def emit_cpp(segments: OrderedDict[str, list[tuple[int, int]]], source: str) -> str:
    ...
    for i, (obj, pixels) in enumerate(segments.items(), 1):
        seg = f"hole17_turfWall_seg{i}"
        names.append(seg)
        world = [to_world(px, py) for px, py in pixels]
        lines += [f"// {obj!r} ({len(world)} vertices)", f"static const float {seg}[][2] = {{"]
        lines += [f"    {{ {x:7.3f}f, {z:7.3f}f}}," for x, z in world]
        lines += ["};", f"static const int {seg}Count = {len(world)};", ""]
    lines += ["static void addHole17TurfWallSegments(TurfWall* wall)", "{"]
...
def main() -> None:
    ...
    if BEGIN not in text:
        text = text.replace("// BEGIN HOLE18_TURF_WALL_DATA", BEGIN + "\n" + END + "\n\n// BEGIN HOLE18_TURF_WALL_DATA", 1)
