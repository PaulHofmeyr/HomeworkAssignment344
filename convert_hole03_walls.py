#!/usr/bin/env python3
"""
Regenerate hole-03 turf wall vertices in TurfWall.cpp from hole-3-turf-wall.csv.

Each CSV object is one separate wall segment (two walls for hole 3).

Usage:
  python3 convert_hole03_walls.py [path-to.csv]
...
DEFAULT_CSV = ROOT / "hole-3-turf-wall.csv"
...
BEGIN = "// BEGIN HOLE03_TURF_WALL_DATA (auto-generated — do not edit by hand)"
END = "// END HOLE03_TURF_WALL_DATA"
...
def emit_cpp(segments: OrderedDict[str, list[tuple[int, int]]], source: str) -> str:
    ...
    lines.append("static void addHole03TurfWallSegments(TurfWall* wall)")
    lines.append("{")
...
def ensure_markers(text: str) -> str:
    if BEGIN in text:
        return text
    needle = "// BEGIN HOLE18_TURF_WALL_DATA"
    if needle not in text:
        raise SystemExit("Could not find HOLE18 marker to insert HOLE03 block")
