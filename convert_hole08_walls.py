#!/usr/bin/env python3
"""
Regenerate hole-08 turf wall vertices in TurfWall.cpp from hole-8-turf-wall.csv.

Each CSV object column value is one separate wall segment (two walls for hole 8).

Usage:
  python3 convert_hole08_walls.py [path-to.csv]
...
DEFAULT_CSV = ROOT / "hole-8-turf-wall.csv"
...
BEGIN = "// BEGIN HOLE08_TURF_WALL_DATA (auto-generated — do not edit by hand)"
END = "// END HOLE08_TURF_WALL_DATA"
...
def emit_cpp(segments: OrderedDict[str, list[tuple[int, int]]], source: str) -> str:
    ...
    lines.append("static void addHole08TurfWallSegments(TurfWall* wall)")
    lines.append("{")
...
def ensure_markers(text: str) -> str:
    if BEGIN in text:
        return text
    needle = "// BEGIN HOLE04_TURF_WALL_DATA"
    if needle not in text:
        raise SystemExit("Could not find HOLE04 marker to insert HOLE08 block")
